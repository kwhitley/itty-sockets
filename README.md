...
<!-- BEGIN SNIPPET -->
```ts
let connect=(e,s={})=>{let a,t=0,n=[],p=[],o={},l=()=>(a||(a=new WebSocket((/^wss?:/.test(e)?e:"wss://ittysockets.io/c/"+e)+"?"+new URLSearchParams(s)),a.onmessage=(e,s=JSON.parse(e.data),a=s?.message,t={...null==a?.[0]&&a,...s,...s.date&&{date:new Date(s.date)}})=>(o[a?.type??s?.type??"message"]?.map((e=>e(t))),p.map((([e,s])=>e(t)&&s(t))),o["*"]?.map((e=>e(t)))),a.onopen=()=>(n.splice(0).map((e=>a?.send(e))),o.open?.map((e=>e())),t&&a?.close()),a.onclose=()=>(t=0,a=null,o.close?.map((e=>e())))),c),c=new Proxy(l,{get:(e,s)=>({open:l,close:()=>(1==a?.readyState?a.close():t=1,c),push:(e,s)=>(t=1,c.send(e,s)),send:(e,s)=>(e=JSON.stringify(e),e=s?"@@"+s+"@@"+e:e,1==a?.readyState?(a.send(e),c):(n.push(e),l())),on:(e,s)=>(s&&(e?.[0]?(o[e]??=[]).push(s):p.push([e,s])),l()),remove:(e,s,a=o[e],t=a?.indexOf(s)??-1)=>(~t&&a?.splice(t,1),l())}[s])});return c};
```
<!-- END SNIPPET -->
