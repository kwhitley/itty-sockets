...
<!-- BEGIN SNIPPET -->
```ts
let connect=(e,s={})=>{let t,a=0,n=[],o={},p=[],l=()=>(t||(t=new WebSocket((/^wss?:/.test(e)?e:"wss://ittysockets.io/c/"+e)+"?"+new URLSearchParams(s)),t.onmessage=(e,s=JSON.parse(e.data),t=s?.message,a={...null==t?.[0]&&t,...s,...s.date&&{date:new Date(s.date)}})=>{o[t?.type??s?.type??"message"]?.map((e=>e(a))),p.map((([e,s])=>e(a)&&s(a))),o.all?.map((e=>e(a)))},t.onopen=()=>{for(;n.length;)t?.send(n.shift());o.open?.map((e=>e())),a&&t?.close()},t.onclose=()=>(a=0,t=null,o.close?.map((e=>e())))),r),r=new Proxy(l,{get:(e,s)=>({open:l,close:()=>(1==t?.readyState?t.close():a=1,r),push:(e,s)=>(a=1,r.send(e,s)),send:(e,s)=>(e=JSON.stringify(e),e=s?"@@"+s+"@@"+e:e,1==t?.readyState?(t.send(e),r):(n.push(e),l())),on:(e,s)=>(s&&(e?.[0]?(o[e]??=[]).push(s):p.push([e,s])),l()),remove:(e,s,t=o[e],a=t?.indexOf(s)??-1)=>(~a&&t?.splice(a,1),l())}[s])});return r};
```
<!-- END SNIPPET -->
